...
save_dir = '/zion/data/TCIA_Dataset/converted/prostate_segmentation'
...
def convert():
    ...
    for casename in os.listdir(data_dir):
        lb_fn = path.join(data_dir, casename, 'ps_mr_label.nii.gz')
        stl_fn = path.join(data_dir, casename, 'mr_ps.stl')
